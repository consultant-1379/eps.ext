...
public class TestEventRouterWithNoneRoutingStrategy {
    ...
    public void shouldResolveStrategyNone() {
        ...
        eventRouter.onEvent("sample Message");
        eventRouter.onEvent("sample Message");
        Assert.assertEquals(eventSubscriber1.getEvents().size(), 0);
        Assert.assertEquals(eventSubscriber2.getEvents().size(), 0);
        Assert.assertEquals(eventSubscriber3.getEvents().size(), 0);
        Assert.assertEquals(eventSubscriber4.getEvents().size(), 0);
        Assert.assertEquals(eventSubscriber5.getEvents().size(), 0);
    }
