...
public class TestEventRouterWithShuffledRoutingStrategy {
    ...
    public void shouldResolveStrategyRoundRobin() {
        ...
        eventRouter.onEvent("sample Message");
        eventRouter.onEvent("sample Message");
        int totalNumberOfEventsDelivered = 0;
        for (int i = 0; i < subscriberArray.length; i++) {
            totalNumberOfEventsDelivered += subscriberArray[i].getEvents().size();
        }
        Assert.assertEquals(5, totalNumberOfEventsDelivered);
    }
